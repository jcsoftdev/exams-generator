...
export class ExamsRepository implements ExamsRepositoryPort {
  ...
  /**
   * `PATCH /exams/:examId` (S4) — renames an exam. Tenant-scoped in the WHERE,
   * like every other exam mutation, so a foreign id simply matches no row
   * (returns `false`) instead of leaking whether it exists.
   *
   * The title used to be write-once, autogenerated as "Examen {grado} —
   * {fecha}": two exams of the same grade on the same day were indistinguishable
   * in a list you search by title (audit 2026-08-15).
   */
  async renameExam(examId: string, tenantId: string, title: string): Promise<boolean> {
  ...
  /**
   * `GET /exams/stock/grades` — how many approved questions each grade level
   * has, under the SAME visibility rule as `getQuestionPool`/`countStock`
   * (`questionVisibility`, never duplicated).
   *
   * Exists so the builder's "Grado" dropdown can say which grades actually
   * have a bank behind them: the catalog offers 12 and, on the seeded data,
   * 11 of them dead-end on "no hay preguntas aprobadas para este grado"
   * AFTER the teacher commits to a choice (audit 2026-08-15). One grouped
   * query, no per-grade fan-out — 12 requests would trip the global
   * ThrottlerGuard.
   *
