...
<div class="grid grid-cols-1 gap-4 lg:grid-cols-[300px_1fr] lg:items-start">
  ...
  <div class="flex flex-col gap-3 rounded-card border border-n200 bg-white p-4">
    ...
    <p class="text-center text-xs text-n500">
  ...
  <!-- Hint panel -->
  <div
    class="flex flex-col items-center gap-3 rounded-card border border-dashed border-n300 p-10 text-center"
  >
    ...
    <p class="max-w-xs text-xs text-n500">
      Al generar verás el progreso en vivo, y puedes cambiar de pantalla sin perderlo. Revísalo
      luego en
      <button type="button" class="font-bold text-primary-600 underline" (click)="goToHistory()">
        Historial</button
      >.
    </p>
  </div>
...
function setup(
  over: { getImpl?: (...a: unknown[]) => unknown; cancelImpl?: (...a: unknown[]) => unknown } = {},
) {
  ...
  TestBed.configureTestingModule({
    ...
    providers: [
      ...
      {
        provide: ActivatedRoute,
        useValue: { snapshot: { paramMap: convertToParamMap({ id: "job-1" }) } },
      },
    ],
  });
...
describe("GenerationJobDetailComponent", () => {
  ...
  it("renders question cards for created ids via listDrafts()", () => {
    ...
    expect(compiled.querySelector('[data-testid="job-question"]')?.textContent).toContain(
      "¿Cuánto es 2+2?",
    );
  });
  ...
  it("does not show the cancel button once the job is terminal", () => {
    const { compiled } = setup({
      getImpl: () => of({ ...RUNNING_JOB, status: "completed" as const }),
    });
...
<div class="flex flex-col gap-3">
  <button
    type="button"
    class="w-fit text-xs font-bold text-n600 hover:text-primary-600"
    (click)="goToHistory()"
  >
    ← Historial
  </button>
  ...
  <div data-testid="job-question" class="rounded-card border border-n200 bg-white p-3">
    <div class="mb-1.5 flex items-center gap-2">
      <span
        class="flex h-5 w-5 items-center justify-center rounded-[7px] bg-ai-bg text-[11px] font-extrabold text-ai-text"
      >
        {{ i + 1 }}
      </span>
    ...
    <div class="mt-1.5 flex flex-wrap gap-x-3 gap-y-1 text-xs text-n600">
      @for (alt of q.alternatives; track $index; let altIndex = $index) { @if (isCorrect(q,
      altIndex)) {
...
<div class="flex flex-col gap-3">
  ...
  >
    ...
    <div class="flex-1 text-sm text-n800">
      {{ job.createdCount }}/{{ job.count }} preguntas @if (job.failedCount > 0) {
