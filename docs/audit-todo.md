...
- [x] **Generación de PDFs síncrona y sin tope de `versionCount`** — solo valida `>= 1`; un valor grande cuelga el API (`exam-generation.service.ts:87-99`, `exams.controller.ts:263-281`). ~~Tope servidor (max 5, como el front)~~ hecho (`MAX_VERSION_COUNT` en `exam-generation.service.ts`). Migrar al patrón BullMQ ya existente queda pendiente — decisión: se difiere a tarea propia, ~1 día real (cola, processor, tabla de jobs, SSE, polling en front), no cabe como parche P0. Ver P1 "fallo parcial deja versiones huérfanas" — mismo fix.
...
- [ ] **Generación de PDFs: fallo parcial deja versiones huérfanas** — versiones 1-2 persisten aunque el call devuelva 422 al fallar la 3 (`exam-generation.service.ts:239-296`); usuario ve "falló" pero hay PDFs. Se resuelve al migrar a job queue (ver P0, diferido).
...
- [ ] Botones crudos fuera de `ButtonComponent` en `exam-review` (toda la página) y `generation-job-detail:2,105-112`. **Diferido**: alcance moderado, no un fix de oleada rápida.
...
- [ ] Cache de preview Typst y `bucketEnsured` son estado in-memory por proceso. **Diferido**: solo aplica al escalar a múltiples réplicas, no hay ese plan hoy — queda como nota, no como fix.
...
- [ ] Sin nav persistente ni "volver arriba" tras el hero — **Diferido**: feature de UI nueva, no un fix de oleada rápida.
